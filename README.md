...
| 公開プロフィール（`app/u/[id]/`） | 実装済み。アイコン・自己紹介・好きな傾向タグ・おすすめシナリオを表示。参考になった合計数は表示名の右に控えめに表示（個別レビューは表示しない） |
...
- `app/u/[id]/`：誰でも閲覧できる公開プロフィール。アイコン・自己紹介・好きな傾向タグ・おすすめシナリオ（お気に入り）を表示。累計の参考になった数は「貢献度」のような特別な名称は付けず、表示名の右に小さく表示するのみに留めている。個別レビューを閲覧する導線は無い設計
